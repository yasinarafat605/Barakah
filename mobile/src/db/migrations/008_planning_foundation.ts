...
export const MIGRATION_008_NAME = '008_planning_foundation';

// Kept in executable form for Expo SQLite. The reference SQL file is the
// canonical checksummed release artifact.
async function applyMigration008(db: DatabaseConnection): Promise<void> {
  await db.execAsync(`
    ...
    CREATE TRIGGER trg_transactions_occurred_on_insert BEFORE INSERT ON transactions
    WHEN NEW.occurred_on IS NOT NULL AND NOT (
      length(NEW.occurred_on) = 10 AND substr(NEW.occurred_on, 5, 1) = '-'
      ...
      AND CAST(substr(NEW.occurred_on, 9, 2) AS INTEGER) BETWEEN 1 AND CASE
        ...
        WHEN (CAST(substr(NEW.occurred_on, 1, 4) AS INTEGER) % 400 = 0 OR
          (CAST(substr(NEW.occurred_on, 1, 4) AS INTEGER) % 4 = 0 AND
           CAST(substr(NEW.occurred_on, 1, 4) AS INTEGER) % 100 <> 0)) THEN 29 ELSE 28 END
    ) BEGIN SELECT RAISE(ABORT, 'MIGRATION_ERR_INVALID_CIVIL_DATE'); END;

    CREATE TRIGGER trg_transactions_occurred_on_default AFTER INSERT ON transactions
    ...
    CREATE TRIGGER trg_transactions_occurred_on_update BEFORE UPDATE ON transactions
    ...
    CREATE TABLE budgets (
      id TEXT PRIMARY KEY NOT NULL, name TEXT,
      ...
      CHECK (ends_on>=starts_on), CHECK (rollover_from_budget_id IS NULL OR rollover_from_budget_id<>id)
    );
    ...
    CREATE UNIQUE INDEX uq_budget_category_active ON budget_categories(budget_id,category_id) WHERE deleted_at IS NULL;
    ...
    CREATE TABLE savings_goals (
      id TEXT PRIMARY KEY NOT NULL, name TEXT NOT NULL, preset_key TEXT,
      target_amount INTEGER NOT NULL CHECK (typeof(target_amount)='integer' AND target_amount>0),
      currency TEXT NOT NULL CHECK (length(currency)=3 AND currency GLOB '[A-Z][A-Z][A-Z]'),
      target_date TEXT, linked_account_id TEXT REFERENCES accounts(id) ON DELETE RESTRICT,
      ...
      updated_at INTEGER NOT NULL CHECK (typeof(updated_at)='integer' AND updated_at>0),
      deleted_at INTEGER CHECK (deleted_at IS NULL OR (typeof(deleted_at)='integer' AND deleted_at>0)),
      CHECK ((lifecycle_status='active' AND completed_at IS NULL) OR (lifecycle_status='completed' AND completed_at IS NOT NULL))
    );
    CREATE TABLE savings_goal_entries (
      ...
      occurred_on TEXT NOT NULL, note TEXT,
      ...
      updated_at INTEGER NOT NULL CHECK (typeof(updated_at)='integer' AND updated_at>0),
      deleted_at INTEGER CHECK (deleted_at IS NULL OR (typeof(deleted_at)='integer' AND deleted_at>0)),
      CHECK ((link_mode='allocation_only' AND transaction_id IS NULL) OR (link_mode IN ('existing_transfer','owned_transfer') AND transaction_id IS NOT NULL)),
      CHECK (cascade_deleted_at IS NULL OR deleted_at IS NOT NULL)
    );
    CREATE UNIQUE INDEX uq_savings_goal_entry_transaction ON savings_goal_entries(transaction_id) WHERE transaction_id IS NOT NULL;
